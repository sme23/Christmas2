...
.global HP_Name_Color
.type HP_Name_Color, %function
HP_Name_Color:
...
.global Write_Growths_To_Battle_Struct
.type Write_Growths_To_Battle_Struct, %function
...
bne		WriteGrowthsLabel1			@ (branch if not equal) using this here because the function is be too long to beq to directly (branch with comparison operator has a shorter range than direct branch)
b		WriteGrowthsGoBack			@ if false, we're done here
WriteGrowthsLabel1:
...
bhi		WriteGrowthsLabel2			@ if it's higher than 99, keep going
b		WriteGrowthsGoBack			@ otherwise, the character's not levelling up and we are done here
WriteGrowthsLabel2:
...
ldr		r0,=GrowthOptionsIDLink	@ bit 1 is set if we should check for fixed growths mode
ldr		r0,[r0]
mov		r1,#0x1
...
beq		WriteGrowthsNormalGrowths		@ if the bit isn't set, we go straight to the normal growths routine
...
cmp		r0,#0
beq		WriteGrowthsNormalGrowths		@ if not set, go to normal growths routine
b		FixedGrowths		@ if it's set, go to fixed growths routine

.ltorg

WriteGrowthsNormalGrowths:
