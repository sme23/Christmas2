...
.macro draw_textID_at tile_x, tile_y, textID=0, width=3, colour=3, growth_func=-1		@growth func is # of growth getter in growth_getters_table; 0=hp, 1=str, 2=skl, etc
  ...
  .ifge \growth_func
  ldr r1,=Growth_Getter_Table	@growth getters table
  ...
  @r1 = are we a player unit
  @mov r1,sp
  @add r1,#0x18
  mov r1,#0
  
  @growth options word and'd with 0x10, so non-zero if stat name color should reflect growth
  ldr		r2, =GrowthOptionsIDLink
  ldr		r2,[r0]
  mov		r3,#0x10		@set if stat name color should reflect growth
  and		r2,r3	
  mov		r1,r8
  ldrb	r1,[r1,#0xB]
  mov		r2,#0xC0
